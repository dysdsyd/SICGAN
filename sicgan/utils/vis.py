...
def print_instances_class_histogram(num_instances, class_names, results):
    ...
    mean_f1_01 = sum(data[3::6]) / num_classes
    mean_f1_03 = sum(data[4::6]) / num_classes
    mean_f1_05 = sum(data[5::6]) / num_classes
    ...
    data.extend([None] * (N_COLS - (len(data) % N_COLS)))
    # Issue here is you need to decrease the number of columns as normal is not available.
    data.extend(
        [
            "per-instance",
            total_num_instances,
    ...
    table = tabulate(
        data,
        headers=["category", "#instances", "chamfer", "F1(0.1)", "F1(0.3)", "F1(0.5)"]   # Removed Normal
        * (N_COLS // 2),
        tablefmt="pipe",
        numalign="left",
        stralign="center",
...
def print_instances_class_histogram_p2m(num_instances, class_names, results):
    ...
    data.extend([None] * (N_COLS - (len(data) % N_COLS)))
    data.extend(
        ["total", total_num_instances, mean_chamfer, mean_f1_1e_4, mean_f1_2e_4]  #Removed Normal
    )
    data.extend([None] * (N_COLS - (len(data) % N_COLS)))
    data.extend(
        [
    ...
    table = tabulate(
        data,
        headers=["category", "#instances", "chamfer", "F1(0.0001)", "F1(0.0002)"] #Removed Normal
        * (N_COLS // 2),
        tablefmt="pipe",
        numalign="left",
        stralign="center",
