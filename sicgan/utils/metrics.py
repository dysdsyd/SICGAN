...
def compare_meshes(
    ...
    pred_points = _sample_meshes(pred_meshes, num_samples_pred) #Normals removed
    gt_points = _sample_meshes(gt_meshes, num_samples_gt)       #Normals removed
    ...
    if torch.is_tensor(pred_points) and torch.is_tensor(gt_points):
        ...
        metrics = _compute_sampling_metrics(
            pred_points, gt_points, thresholds, eps     #Remove pred&gt normals
        )
    else:
...
def _sample_meshes(meshes, num_samples):
    ...
    if num_samples == "verts":
        ...
        verts = sample_points_from_meshes(meshes, num_samples, return_normals=False)  #Normals removed
    return verts


def _compute_sampling_metrics(pred_points, gt_points, thresholds, eps):
    """
