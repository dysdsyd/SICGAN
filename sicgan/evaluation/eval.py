...
import sicgan.utils.vis as vis_utils
from sicgan.data.utils import image_to_numpy, imagenet_deprocess
...
def evaluate_test(model, data_loader, vis_preds=False):
    ...
    vis_utils.print_instances_class_histogram(
        num_instances,
        class_names,
        {"chamfer": chamfer, "f1_01": f1_01, "f1_03": f1_03, "f1_05": f1_05},  #Removed Normal
    )
...
def evaluate_test_p2m(model, data_loader):
    ...
    for batch in data_loader:
        ...
        # with inference_context(model):
        meshes_pred = model(imgs)     #Removed Voxels
    ...
    vis_utils.print_instances_class_histogram_p2m(
        num_instances,
        class_names,
        {"chamfer": chamfer, "f1_1e_4": f1_1e_4, "f1_2e_4": f1_2e_4},  #Remove Normal
    )
...
def evaluate_split(
    ...
    for batch in loader:
        ...
        imgs, meshes_gt, points_gt, normals_gt = batch
        meshes_pred = model(imgs)
